...
The RPM spec file provided here is for Ion-Core 0.12.0 and builds on CentOS
...
in the ion git repository. At present only an SVG version of the Ion
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the ion
...
needed when 0.12.1 is released, a proper fix is already in the Ion git
master branch.
...
idea to build Ion Core against a static version of that library compiled
...
The `ion.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `ion-server` package. At this point, most RPM based
...
The `ion-server` package should have SELinux support. How to properly do
...
Ion-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/ion/). He has been
packaging Ion for Fedora at least since 2012.
...
4. I renamed the `ion` package that contains the Qt GUI to `ion-core` as
that appears to be how the general population refers to it, in contrast to
`ion-xt` or `ion-classic`. I wanted to make sure the general population
