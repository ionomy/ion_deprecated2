...
See the section "Disable-wallet mode" to build Ion Core without wallet.
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, ion may be compiled in
...
`--with-incompatible-bdb` according to the [PKGBUILD](https://projects.archlinux.org/svntogit/community.git/tree/ion/trunk/PKGBUILD).
As mentioned above, when maintaining portability of the wallet between the standard Ion Core distributions and independently built
...
with 4.8-built Ion Core is needed follow the steps under "Berkeley DB" above.
