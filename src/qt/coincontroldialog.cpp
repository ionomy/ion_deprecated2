    ...
    model(0)
{
    ...
    // see https://github.com/cevap/ion/issues/5716
...
{
    GUIUtil::setClipboard(IonUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
        ...
        if (item->checkState(COLUMN_CHECKBOX) == Qt::Unchecked)
            ...
            if(coinControl->useStashedSend && rounds < nStashedsendRounds) {
                QMessageBox::warning(this, windowTitle(),
                    tr("Non-anonymized input selected. <b>Stashedsend will be disabled.</b><br><br>If you still want to use Stashedsend, please deselect all non-nonymized inputs first and then check Stashedsend checkbox again."),
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    l2->setText(IonUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(IonUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(IonUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));                                    // Bytes
    l6->setText(sPriorityLabel);                                             // Priority
    l7->setText((fLowOutput ? (fDust ? tr("DUST") : tr("yes")) : tr("no"))); // Low Output / Dust
    l8->setText(IonUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    ...
    // tool tips
    l5->setToolTip(tr("This label turns red, if the transaction size is bigger than 10000 bytes.\n\n This means a fee of at least %1 per kb is required.\n\n Can vary +/- 1 Byte per input.").arg(IonUnits::formatWithUnit(nDisplayUnit, CENT)));
    l6->setToolTip(tr("Transactions with higher priority get more likely into a block.\n\nThis label turns red, if the priority is smaller than \"medium\".\n\n This means a fee of at least %1 per kb is required.").arg(IonUnits::formatWithUnit(nDisplayUnit, CENT)));
    l7->setToolTip(tr("This label turns red, if any recipient receives an amount smaller than %1.\n\n This means a fee of at least %2 is required. \n\n Amounts below 0.546 times the minimum relay fee are shown as DUST.").arg(IonUnits::formatWithUnit(nDisplayUnit, CENT)).arg(IonUnits::formatWithUnit(nDisplayUnit, CENT)));
    l8->setToolTip(tr("This label turns red, if the change is smaller than %1.\n\n This means a fee of at least %2 is required.").arg(IonUnits::formatWithUnit(nDisplayUnit, CENT)).arg(IonUnits::formatWithUnit(nDisplayUnit, CENT)));
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, IonUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, IonUnits::format(nDisplayUnit, nSum));
