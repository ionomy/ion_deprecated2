...
{
    QString ret = QString("ion:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ION unit input in lnReqAmount, we generate the URI with ION as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(IonUnits::format(IonUnits::ION, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
