...
def check_json_precision():
    """Make sure json library being used does not lose precision converting ION values"""
...
def wait_for_iond_start(process, url, i):
    '''
    Wait for iond to start. This means that RPC is accessible and fully initialized.
    Raise an exception if iond exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('iond exited with status %i during initialization' % process.returncode)
        try:
...
def _start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, stderr=None):
    ...
    iond_processes[i] = subprocess.Popen(args, stderr=stderr)
    logger.debug("initialize_chain: iond started, waiting for RPC to come up")
    url = rpc_url(i, rpchost)
    wait_for_iond_start(iond_processes[i], url, i)
...
def _stop_node(node, i):
    ...
    return_code = iond_processes[i].wait(timeout=IOND_PROC_WAIT_TIMEOUT)
...
def _stop_nodes(nodes):
    ...
    for i, node in enumerate(nodes):
        _stop_node(node, i)
    assert not iond_processes.values() # All connections must be gone now
