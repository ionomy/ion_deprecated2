...
class IonTestFramework(object):
    """Base class for a ion test script.

    Individual ion test scripts should subclass this class and override the following methods:
    ...
    def _start_logging(self):
        ...
        if self.options.trace_rpc:
            rpc_logger = logging.getLogger("IonRPC")
    ...
    def _initialize_chain(self, test_dir, num_nodes, cachedir):
        ...
        if create_cache:
            ...
            for i in range(MAX_NODES):
                ...
                args = [os.getenv("IOND", "iond"), "-server", "-keypool=1", "-datadir=" + datadir, "-discover=0"]
                if i > 0:
                    args.append("-connect=127.0.0.1:" + str(p2p_port(0)))
                iond_processes[i] = subprocess.Popen(args)
                self.log.debug("initialize_chain: iond started, waiting for RPC to come up")
                wait_for_iond_start(iond_processes[i], rpc_url(i), i)
...
# Test framework for doing p2p comparison testing, which sets up some iond
# binaries:
...
class ComparisonTestFramework(IonTestFramework):
